...
int main(int argc, char *argv[]) {
	...
	if (argc != 11 && argc != 12) {
		printf(RED("Usage: %s REQUEST_SEQUENCE DISTANCE_MATRIX TIME_MATRIX SEED TIME_BUDGET GENERATION_PORTION DETERMINISM_RATE CANDIDATE_LIST_LENGTH #THREADS ENV_BENEFIT_K\n"), argv[0]);
		return EXIT_FAILURE;
	}
	...
	read_matrix(argv[2], Z, Z, distance);
	read_matrix(argv[3], Z, Z, time);
	const uint seed = atoi(argv[4]);
	const float tb = atof(argv[5]) * 1e3; // Milliseconds
	const float gp = atof(argv[6]);
	const float dr = atof(argv[7]);
	const uint cl = if_not_zero(atoi(argv[8]), UINT_MAX);
	const uint n_threads = if_not_zero(atoi(argv[9]), omp_get_max_threads());
	const float env_k = atof(argv[10]);
	...
	for (uint i = 0; i < n_threads; ++i) {
		gen[i].seed(seed + i);
		data[i] = { n, PTR(reqs), PTR(steps), PTR(deltas), PTR(distance), PTR(time), .vars = &vars[i],
			    couples, paths, drs, qoss, PTR(feasible),
			    PTR(ur), PTR(ui), .gen = &gen[i],
			    gp * tb, dr, cl, env_k };
	}
