...
def f(idx, p):
    # return abs(sum(2 * (p[idx][:, 0] % 2) - 1))
    
    segments = p[idx].sort(dim=1)[0].sort(dim=0)[0]
    union = [[0, 0]]
    for seg in segments:
        if seg[0] > union[-1][1]:
            union.append(seg)
        else:
            union[-1][1] = max(union[-1][1], seg[1])
    return sum(b - a for a, b in union)

    # segments = p[idx].sort(dim=1)[0]
    # intersect = [0, 62]
    # for seg in segments:
    #     low = max(intersect[0], seg[0])
    #     high = min(intersect[1], seg[1])
    #     intersect = [low, max(low, high)]
...
def get_random_partial_collective(participants, device):
    ...
    # collective_size = torch.randint(MAX_COLLECTIVE_SIZE, size=(1,), device=device).repeat(batch_size, 1)
