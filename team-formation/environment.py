...
from function import carachteristic_function as f
...
class Collective:
    def __init__(self, oracle_data, participants, device):
    ...
    def add_participant(self, action):
        ...
        collective = Collective(self.oracle_data, self.participants, self._device)
    ...
    def get_reward(self):
        ...
        for b, compute in enumerate(compute_reward):
            if compute:
                ...
                reward[b] = f(self.oracle_data, idx.cpu(), p)

        return reward

def get_random_partial_collective(oracle_data, participants, device):
    batch_size = len(participants)
    pool_size = [len(p) for p in participants]

    collective = Collective(oracle_data, participants, device)
    indices = torch.cat([
