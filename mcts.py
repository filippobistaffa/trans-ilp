...
class Coalition(Node):
    def __init__(self, idxs, terminal):
        ...
        if self.idxs:
            self.rw = oracle(np.array(self.idxs, dtype=np.uint32), reqs, steps, deltas, distance, time)
        else:
            self.rw = 0
    ...
    def remaining(self):
        if partial:
            ...
            feasible = all_idxs[:len(all_idxs) - (max_size - len(self.idxs) - 1)]
        remaining = [i for i in feasible if not self.idxs or i > max(self.idxs)]
